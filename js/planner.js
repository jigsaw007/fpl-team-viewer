...
const PL_MAX_CLUB=3;
let _plInit=false,_plPlans=[],_plActiveId=null,_plActiveGw=null,_plFixtures=[],_plOutId=null,_plQuery="",_plPos=0,_plTeam=0;
...
function plMakeWeeks(start){
  const out=[]; for(let gw=start;gw<=38&&out.length<8;gw++) out.push({gw,transfers:[],captain:null,vice:null,chip:""});
  return out;
}
...
function plNormalize(plan){
  ...
  plan.weeks=plMakeWeeks(plan.baseGw).map(w=>{
    ...
    x.gw=w.gw;x.transfers=Array.isArray(x.transfers)?x.transfers:[];x.captain=x.captain?Number(x.captain):null;x.vice=x.vice?Number(x.vice):null;x.chip=x.chip||"";
    return x;
  });
  return plan;
}
...
function plRenderWorkspace(){
  ...
  $("plResetGw").disabled=!(w.transfers?.length||w.captain||w.vice||w.chip);
  $("plSummary").innerHTML=`
...
function plPitchPlayer(e,st,w,{starter=false,baseIncomplete=false,bench=false}={}){
  const t=boot.teams.find(x=>x.id===e.team)||{},isOut=+_plOutId===e.id,cap=+w.captain===e.id,vice=+w.vice===e.id;
  const fixture=plFixtureHtml(e,_plActiveGw);
  const action=baseIncomplete?"Remove from base squad":"Plan transfer";
  return `<div class="planner-pitch-player ${starter?"starter":""} ${bench?"bench":""} ${isOut?"selected-out":""}">
    ...
    ${!baseIncomplete?`<span class="planner-pitch-actions"><button class="planner-pitch-arm ${cap?"active":""}" data-pl-cap="${e.id}" title="Captain ${esc(e.web_name)}">C</button><button class="planner-pitch-arm vice ${vice?"active":""}" data-pl-vice="${e.id}" title="Vice captain ${esc(e.web_name)}">V</button></span>`:""}
...
function plRenderSquad(st,w){
  ...
  const xi=new Set(plAutoXi(st.squad));
  ...
  $("plSquad").innerHTML=`
    <div class="planner-pitch-topline"><div><span>Starting XI</span><b>${formation}</b></div><small>Auto-arranged preview · click a player to plan a transfer</small></div>
    ...
    <div class="planner-fpl-bench"><div class="planner-bench-title"><span>Bench</span><small>Auto order</small></div><div class="planner-bench-row">${benchOrder.map(e=>plPitchPlayer(e,st,w,{bench:true})).join("")}</div></div>`;
}
...
function plAddOrTransfer(id){
  ...
  const w=plWeek(plan,_plActiveGw);w.transfers.push({id:plUuid(),out:out.id,in:incoming.id,sellPrice:sell,buyPrice:buy});if(w.captain===out.id)w.captain=null;if(w.vice===out.id)w.vice=null;_plOutId=null;plTouch(plan);plRenderAll();
}
function plSetCaptain(id,type){
  const plan=plActive(),w=plWeek(plan,_plActiveGw),st=plDerive(plan,_plActiveGw);id=+id;if(!st.squad.includes(id))return;
  if(type==="cap"){w.captain=w.captain===id?null:id;if(w.captain===w.vice)w.vice=null}else{w.vice=w.vice===id?null:id;if(w.vice===w.captain)w.captain=null}plTouch(plan);plRenderAll();
}
...
function plResetGw(){const plan=plActive(),w=plWeek(plan,_plActiveGw);if(!w)return;w.transfers=[];w.captain=null;w.vice=null;w.chip="";_plOutId=null;plTouch(plan);plRenderAll()}
...
async function initPlanner(){
  ...
  $("plPlanSelect").addEventListener("change",e=>{_plActiveId=e.target.value||null;_plActiveGw=plActive()?.baseGw||null;_plOutId=null;plRenderAll()});
  ...
  $("plGwStrip").addEventListener("click",e=>{const b=e.target.closest("[data-gw]");if(!b)return;_plActiveGw=+b.dataset.gw;_plOutId=null;plRenderAll()});
  $("plChip").addEventListener("change",e=>{const p=plActive(),w=plWeek(p,_plActiveGw);w.chip=e.target.value;plTouch(p);plRenderGwStrip();plRenderWorkspace()});$("plResetGw").addEventListener("click",plResetGw);
  ...
  $("plSquad").addEventListener("click",e=>{const c=e.target.closest("[data-pl-cap]");if(c)return plSetCaptain(c.dataset.plCap,"cap");const v=e.target.closest("[data-pl-vice]");if(v)return plSetCaptain(v.dataset.plVice,"vice");const empty=e.target.closest("[data-pl-empty-pos]");if(empty){_plPos=+empty.dataset.plEmptyPos;$("plannerPos").value=String(_plPos);plRenderPicker(plDerive(plActive(),_plActiveGw));$("plannerSearch")?.focus();return}const out=e.target.closest("[data-pl-out]");if(out)return plSetOut(out.dataset.plOut)});
